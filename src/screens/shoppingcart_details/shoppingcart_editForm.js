...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], shoppingCartId, shoppingcart, numfields, mapItem;

        shoppingcart = row['shoppingcart'];
        shoppingCartId = row['shoppingcart'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) shoppingCartId = rslt.id;
                            
                            const mapPropKey = shoppingcart.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "CartId", value: parseInt(shoppingCartId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateShoppingCart(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'shoppingcart');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "ShoppingCart is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
