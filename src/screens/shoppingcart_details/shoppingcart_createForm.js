...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, shoppingCartId, numfields;
        const mapItems = MapItems;

        let shoppingcart = row['shoppingcart'];

        numfields = Helper.GetAllNumberFields(shoppingcart);
        if (numfields.length > 0) Helper.UpdateNumberFields(shoppingcart, numfields);


        // Add Or Update ShoppingCart
        rslt = await Support.AddOrUpdateShoppingCart(shoppingcart, []);
        if (rslt.status) {
            shoppingCartId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the ShoppingCart and navigation entity if it is deos not exist
            let navItem = shoppingcart.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "CartId", value: parseInt(shoppingCartId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateShoppingCart(data);
        ...
        global.AlertPopup("success", "ShoppingCart is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
