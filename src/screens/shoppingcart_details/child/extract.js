import ShoppingCartJsonConfig from "config/shoppingcart_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "shoppingcart", expand: "", exclude: [], func: Support.AddOrUpdateShoppingCart }
]

const FetchShoppingCartInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(ShoppingCartJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of ShoppingCartJsonConfig[elm]) {
...
const FetchShoppingCartDetails = async (shoppingcartId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (shoppingcartId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetShoppingCartSingle(shoppingcartId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetShoppingCartSingle(shoppingcartId);
            }
            
            if (rslt.status) {

                const shoppingcart = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? shoppingcart : shoppingcart[source];
...
const Extract = async (shoppingcartId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchShoppingCartInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(shoppingcartId)) {
                    await FetchShoppingCartDetails(shoppingcartId, enums).then(({ row, backRow }) => {
