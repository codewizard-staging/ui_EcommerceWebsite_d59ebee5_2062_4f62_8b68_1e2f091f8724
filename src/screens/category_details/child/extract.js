import CategoryJsonConfig from "config/category_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "category", expand: "", exclude: [], func: Support.AddOrUpdateCategory }
]

const FetchCategoryInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(CategoryJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of CategoryJsonConfig[elm]) {
...
const FetchCategoryDetails = async (categoryId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (categoryId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetCategorySingle(categoryId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetCategorySingle(categoryId);
            }
            
            if (rslt.status) {

                const category = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? category : category[source];
...
const Extract = async (categoryId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchCategoryInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(categoryId)) {
                    await FetchCategoryDetails(categoryId, enums).then(({ row, backRow }) => {
