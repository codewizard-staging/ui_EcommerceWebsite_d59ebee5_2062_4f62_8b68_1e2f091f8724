...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, categoryId, numfields;
        const mapItems = MapItems;

        let category = row['category'];

        numfields = Helper.GetAllNumberFields(category);
        if (numfields.length > 0) Helper.UpdateNumberFields(category, numfields);


        // Add Or Update Category
        rslt = await Support.AddOrUpdateCategory(category, []);
        if (rslt.status) {
            categoryId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Category and navigation entity if it is deos not exist
            let navItem = category.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "CategoryId", value: parseInt(categoryId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateCategory(data);
        ...
        global.AlertPopup("success", "Category is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
