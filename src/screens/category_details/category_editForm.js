...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], categoryId, category, numfields, mapItem;

        category = row['category'];
        categoryId = row['category'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) categoryId = rslt.id;
                            
                            const mapPropKey = category.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "CategoryId", value: parseInt(categoryId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateCategory(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'category');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Category is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
