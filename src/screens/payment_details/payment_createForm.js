...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, paymentId, numfields;
        const mapItems = MapItems;

        let payment = row['payment'];

        numfields = Helper.GetAllNumberFields(payment);
        if (numfields.length > 0) Helper.UpdateNumberFields(payment, numfields);


        // Add Or Update Payment
        rslt = await Support.AddOrUpdatePayment(payment, []);
        if (rslt.status) {
            paymentId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Payment and navigation entity if it is deos not exist
            let navItem = payment.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "PaymentId", value: parseInt(paymentId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdatePayment(data);
        ...
        global.AlertPopup("success", "Payment is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
