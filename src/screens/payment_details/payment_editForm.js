...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], paymentId, payment, numfields, mapItem;

        payment = row['payment'];
        paymentId = row['payment'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) paymentId = rslt.id;
                            
                            const mapPropKey = payment.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "PaymentId", value: parseInt(paymentId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdatePayment(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'payment');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Payment is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
