import PaymentJsonConfig from "config/payment_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "payment", expand: "", exclude: [], func: Support.AddOrUpdatePayment }
]

const FetchPaymentInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(PaymentJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of PaymentJsonConfig[elm]) {
...
const FetchPaymentDetails = async (paymentId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (paymentId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetPaymentSingle(paymentId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetPaymentSingle(paymentId);
            }
            
            if (rslt.status) {

                const payment = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? payment : payment[source];
...
const Extract = async (paymentId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchPaymentInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(paymentId)) {
                    await FetchPaymentDetails(paymentId, enums).then(({ row, backRow }) => {
