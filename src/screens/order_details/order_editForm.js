...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], orderId, order, numfields, mapItem;

        order = row['order'];
        orderId = row['order'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) orderId = rslt.id;
                            
                            const mapPropKey = order.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "OrderId", value: parseInt(orderId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateOrder(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'order');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
