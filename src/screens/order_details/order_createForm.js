...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, orderId, numfields;
        const mapItems = MapItems;

        let order = row['order'];

        numfields = Helper.GetAllNumberFields(order);
        if (numfields.length > 0) Helper.UpdateNumberFields(order, numfields);


        // Add Or Update Order
        rslt = await Support.AddOrUpdateOrder(order, []);
        if (rslt.status) {
            orderId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Order and navigation entity if it is deos not exist
            let navItem = order.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "OrderId", value: parseInt(orderId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateOrder(data);
