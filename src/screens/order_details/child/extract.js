import OrderJsonConfig from "config/order_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "order", expand: "", exclude: [], func: Support.AddOrUpdateOrder }
]
...
const FetchOrderDetails = async (orderId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (orderId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetOrderSingle(orderId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetOrderSingle(orderId);
            }
            ...
            if (rslt.status) {
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? order : order[source];
        ...
        await FetchOrderInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(orderId)) {
                    await FetchOrderDetails(orderId, enums).then(({ row, backRow }) => {
