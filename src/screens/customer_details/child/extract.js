import CustomerJsonConfig from "config/customer_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "customer", expand: "", exclude: [], func: Support.AddOrUpdateCustomer }
]
    ...
    return new Promise(async (resolve) => {
        ...
        if (customerId) {
            ...
            if ($expand.length > 0) {
                ...
                rslt = await Api.GetCustomerSingle(customerId);
            }
        ...
        await FetchCustomerInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(customerId)) {
                    await FetchCustomerDetails(customerId, enums).then(({ row, backRow }) => {
