...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], customerId, customer, numfields, mapItem;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) customerId = rslt.id;
                            
                            const mapPropKey = customer.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'customer');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Customer is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
