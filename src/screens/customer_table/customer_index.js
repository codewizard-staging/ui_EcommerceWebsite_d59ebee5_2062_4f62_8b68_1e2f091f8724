...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetCustomersCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        await Api.GetCustomersMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetCustomerSingle({ CustomerId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Customers/edit/${id}`;
        if (type === 'view') _route = `/Customers/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'CustomerId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
