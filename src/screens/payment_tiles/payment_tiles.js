...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetPaymentsCount(query)
            .then(async (res) => {
                if (res.status) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetPaymentSingle({ PaymentId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Payments/edit/${id}`;
        if (type === 'view') _route = `/Payments/view/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Payments/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
