        ...
        <>
            <GridContainer>
                {rows && rows.map((x, index) => (
                    <React.Fragment key={`${x.CategoryId}_${index}`} >
                        <CardItem keyid={x.CategoryId} row={x} title={x.$titleProp}  imgsrc={x.CategoryCategoryNameData}  width={300}
                            footerItems={[]} description={x.$descProp} onActionClicked={OnActionClicked}>
