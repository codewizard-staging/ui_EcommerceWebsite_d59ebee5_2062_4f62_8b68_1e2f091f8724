import ProductJsonConfig from "config/product_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "product", expand: "", exclude: [], func: Support.AddOrUpdateProduct }
]
...
const FetchProductDetails = async (productId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (productId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetProductSingle(productId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetProductSingle(productId);
            }
