...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetShoppingCartsCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        let _rows = [];
        await Api.GetShoppingCartsMulti(
            query, 
            ""
            )
            .then(async (res) => {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetShoppingCartSingle({ CartId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/ShoppingCarts/edit/${id}`;
        if (type === 'view') _route = `/ShoppingCarts/view/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/ShoppingCarts/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
